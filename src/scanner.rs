...
use walkdir::WalkDir;
use crate::types::{MediaFile, MediaType, ScanResult, ScanStatistics, FailedFile};
...
impl Scanner {
    ...
    pub async fn scan_directory(&self, directory: &Path) -> Result<ScanResult> {
        ...
        let statistics = self.calculate_statistics(&media_files, &parsed_files, &failed_files, start_time);
    ...
    fn detect_media_type(&self, file_path: &Path, extension: &str) -> Result<MediaType> {
        ...
        if matches!(extension, "mp3" | "flac" | "wav" | "m4a" | "aac" | "ogg" | "wma") {
        ...
        if let Some(parent) = file_path.parent() {
            ...
            if parent_name.contains("season") || parent_name.contains("episode") || 
               (parent_name.contains("s") && parent_name.contains("e") && 
                (parent_name.contains("season") || parent_name.contains("episode"))) {
                return Ok(MediaType::TvShow);
            }
            
            // Check for movie indicators
            if parent_name.contains("movie") || parent_name.contains("movies") || parent_name.contains("film") {
    ...
    async fn parse_media_files(&self, media_files: &[MediaFile]) -> Result<(Vec<crate::types::ParsingResult>, Vec<FailedFile>)> {
    ...
    async fn parse_single_file(&self, media_file: &MediaFile) -> Result<crate::types::ParsingResult> {
        match media_file.media_type {
            MediaType::Movie => {
                self.movie_parser.parse_movie(&media_file.file_path).await
            }
            MediaType::TvShow => {
    ...
    fn calculate_statistics(
        ...
        let average_confidence = if !parsed_files.is_empty() {
            let total_confidence: f32 = parsed_files.iter()
